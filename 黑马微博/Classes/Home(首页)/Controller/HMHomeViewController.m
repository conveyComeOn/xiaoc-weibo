...
- (void)loadNewStatuses:(UIRefreshControl *)refreshControl{
   ...
   HMStatusFrame *statue=[self.statusFrames firstObject];
    HMStatus *frist=statue.status;
    if (frist) {
        
        dict[@"since_id"]=@([frist.idstr longLongValue]);
    }
    
    NSLog(@"rr");
    
    [request GET:@"https://api.weibo.com/2/statuses/home_timeline.json" parameters:dict success:^(AFHTTPRequestOperation *operation, id responseObject) {
        // 微博字典数组
        ...
        // 微博字典数组 ---> 微博模型数组
        NSArray *arra = [HMStatus objectArrayWithKeyValuesArray:statusDictArray];
        NSArray *newFrame=[self statusFramesWithStatuses:arra];
        
        NSRange rang=NSMakeRange(0, newFrame.count);
        NSIndexSet *set=[NSIndexSet indexSetWithIndexesInRange:rang];
        [self.statusFrames insertObjects:newFrame atIndexes:set];
        
        [self.tableView reloadData];
        
        [refreshControl endRefreshing];
        [self showNewStatusesCount:arra.count];
...
- (void)loadMoreStatuses
{
    ...
    HMStatusFrame *lastStatus =  [self.statusFrames lastObject];
    HMStatus *last=lastStatus.status;
    if (last) {
        params[@"max_id"] = @([last.idstr longLongValue] - 1);
    }
    ...
    [HMHttpTool get:@"https://api.weibo.com/2/statuses/home_timeline.json" params:params success:^(id responseObj) {
        ...
        NSArray *newStatusesframe=[self statusFramesWithStatuses:newStatuses];
        // 将新数据插入到旧数据的最后面
        [self.statusFrames addObjectsFromArray:newStatusesframe];
...
//    // 0.清零提醒数字
    ...
    // 2.显示文字
    if (count) {
        label.text = [NSString stringWithFormat:@"共有%d条新的葱博", count];
    } else {
        label.text = @"没有最新的葱博";
    }
...
//    imageview.image=[UIImage imageNamed:[NSString stringWithFormat:@"xiaocong%d",arc4random_uniform(11)]];
//    imageview.frame=newVc.view.bounds;
//    [newVc.view addSubview:imageview];
//    newVc.title = @"别看人家";
//    [self.navigationController pushViewController:newVc animated:YES];
    KWDetileStatus *detile=[[KWDetileStatus alloc]init];
    HMStatusDetailFrame *frame=self.statusFrames[indexPath.row];
    
    frame.status.retweeted_status.detile=YES;
    detile.status=frame.status;
