...
- (void)setEmotions:(NSArray *)emotions{
    ...
    HMEmotion *emo=emotions[0];
    [self.scrollView.subviews makeObjectsPerformSelector:@selector(removeFromSuperview)];

    if ([emo.directory isEqualToString:@"EmotionIcons/xiaocong"]) {
        self.pageControl.numberOfPages=emotions.count/3;
    }
    int totalPages=self.pageControl.numberOfPages;
    // 决定scrollView显示多少页表情
    for (int i = 0; i<totalPages; i++) {
      ...
      Range.location = i * HMEmotionMaxCountPerPage;
         Range.length= HMEmotionMaxCountPerPage;
        }
        if (Range.length + Range.location > emotions.count) { // 对越界进行判断处理
